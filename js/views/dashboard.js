  ...
  <section class="max-w-[1280px] mx-auto px-5 md:px-10 py-8">
    <div class="flex items-end justify-between mb-7">
      ...
      <div class="flex gap-2">
        <a href="#/employees" class="btn btn-soft">직원 관리</a>
        <a href="#/admin/new-grant" class="btn btn-primary">부여 등록</a>
        <a href="#/admin/csv" class="btn btn-soft">CSV 업로드</a>
      </div>
    </div>
    <div id="cards" class="grid grid-cols-1 md:grid-cols-4 gap-5"></div>
  </section>`;
}

export async function bindDashboard(){
  const gs = await getDocs(collection(db, COL.GRANTS));
  ...
  const es = await getDocs(collection(db, COL.EMP));
  const empCount = es.size;

  document.getElementById("cards").innerHTML = `
    <div class="hr-card p-6"><p class="text-muted text-sm">직원 수</p><p class="text-4xl font-extrabold">${fmt.int(empCount)}</p></div>
